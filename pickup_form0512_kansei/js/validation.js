document.addEventListener('DOMContentLoaded', () => {
  ...
  if(validationForm) {
    ...
    validationForm.addEventListener('submit', (e) => {
      ...
      addressElems.forEach( (elem) => {
        ...
        if(elem.value !=='') {
          ...
          if(!pattern.test(elem.value)) {
            createError(elem, '※郵便番号をハイフン込で正しく入力してください');
            e.preventDefault();
          }
        }
      });
      ...
      if(errorElem) {
        ...
        window.scrollTo({
          top: errorElemOffsetTop - 200,  //200px 上に位置を調整
          //スムーススクロール
          behavior: 'smooth'
        });
      }
  ...
  //チェックボックスにチェックが入ったらボタンを活性化
  const targetButton = document.getElementById('submit_btn');
  const triggerCheckbox = document.getElementById('privacypolicy');;
